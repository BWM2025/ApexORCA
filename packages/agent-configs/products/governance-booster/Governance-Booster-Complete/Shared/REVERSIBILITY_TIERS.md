...
**Rule**: Agent must NOT execute without founder approval (or CEO-Lead approval when delegated). Propose action with clear impact; wait for explicit go.
...
- Governance-Lead (when present) enforces classification on audit; misclassified actions trigger halt and escalation.
